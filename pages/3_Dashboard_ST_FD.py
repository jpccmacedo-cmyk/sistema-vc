...
def tabela_heatmap_html(tabela, tipo="ST", ampliado=False):
    ...
    table.heatmap-table {{ border-collapse:collapse; font-family:Arial, Helvetica, sans-serif; font-size:{font_size}; background:white; color:#000000; }}
    .heatmap-table th, .heatmap-table td {{ border:1px solid #D9E2EF; padding:{pad}; text-align:center; white-space:nowrap; min-width:{min_w}; height:23px; line-height:1.1; font-weight:400; }}
...
def render_heatmap_png(tabela, tipo="ST", titulo="Heatmap"):
    if tabela.empty:
        return None
    ...
    ax.text(n_cols / 2, n_rows + 1.05, titulo, ha="center", va="center", fontsize=15, fontweight="bold", color="#0F172A")

    header_y = n_rows
    for c, col in enumerate(tabela.columns):
        ax.add_patch(Rectangle((c, header_y), 1, 0.55, facecolor="#F8FAFC", edgecolor="#D9E2EF", linewidth=0.7))
        ax.text(c + 0.5, header_y + 0.275, str(col), ha="center", va="center", fontsize=7.2, fontweight="normal", color="#0F172A")
    ...
    for r, (_, row) in enumerate(tabela.iterrows()):
        ...
        for c, col in enumerate(tabela.columns):
            ...
            ax.add_patch(Rectangle((c, y), 1, 1, facecolor=bg, edgecolor="#D9E2EF", linewidth=0.7))
            ax.text(c + 0.5, y + 0.5, txt, ha="center", va="center", fontsize=7.2, fontweight="normal", color="#000000")
...
with tab_st:
    ...
    if tabela_st.empty:
        ...
        html_st = tabela_heatmap_html(tabela_st, tipo="ST", ampliado=False)
        st.markdown(html_st, unsafe_allow_html=True)
        png_st = render_heatmap_png(tabela_st, tipo="ST", titulo=titulo_st)
        with st.expander("Visualizacao completa para print"):
            st.image(png_st, use_container_width=True)
        col_download_1, col_download_2 = st.columns(2)
        with col_download_1:
            st.download_button("Baixar ST Heatmap em PNG", data=png_st, file_name="st_heatmap.png", mime="image/png")
        with col_download_2:
            st.download_button("Baixar ST Heatmap em HTML", data=html_document(titulo_st, html_st).encode("utf-8"), file_name="st_heatmap.html", mime="text/html")
...
with tab_fd:
    ...
    if tabela_fd.empty:
        ...
        html_fd = tabela_heatmap_html(tabela_fd, tipo="FD", ampliado=False)
        st.markdown(html_fd, unsafe_allow_html=True)
        png_fd = render_heatmap_png(tabela_fd, tipo="FD", titulo=titulo_fd)
        with st.expander("Visualizacao completa para print"):
            st.image(png_fd, use_container_width=True)
        col_download_1, col_download_2 = st.columns(2)
        with col_download_1:
            st.download_button("Baixar FD Multiflex em PNG", data=png_fd, file_name="fd_multiflex_heatmap.png", mime="image/png")
        with col_download_2:
            st.download_button("Baixar FD Multiflex em HTML", data=html_document(titulo_fd, html_fd).encode("utf-8"), file_name="fd_multiflex_heatmap.html", mime="text/html")
