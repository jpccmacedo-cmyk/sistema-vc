...
# ============================================================
# Cores, limpeza e tabelas
# ============================================================

def limpar_none_tabela(tabela):
    if tabela.empty:
        return tabela
...
def montar_tabela_st(df_mes):
    ...
    for planta in PLANTAS_ORDEM:
        ...
        linha = {
            "Planta": planta,
            "M300": bloco["m300"].dropna().iloc[0] if bloco["m300"].notna().any() else pd.NA,
            "RF": bloco["rf"].dropna().iloc[0] if bloco["rf"].notna().any() else pd.NA,
        }
        for _, row in bloco.iterrows():
            linha[row["data_label"]] = row["valor_real"] if pd.notna(row["valor_real"]) else pd.NA
        linha["Acc mes"] = bloco["acumulado_mes"].dropna().iloc[0] if bloco["acumulado_mes"].notna().any() else pd.NA
        linhas.append(linha)

    return limpar_none_tabela(pd.DataFrame(linhas))


def estilizar_st(tabela):
    tabela = limpar_none_tabela(tabela)
...
def montar_tabela_fd(df_mes):
    ...
    for planta in PLANTAS_ORDEM:
        ...
        for _, row in bloco.iterrows():
            linha[row["data_label"]] = row["valor_real"] if pd.notna(row["valor_real"]) else pd.NA
        linha["Real TT"] = bloco["acumulado_mes"].dropna().iloc[0] if bloco["acumulado_mes"].notna().any() else pd.NA
        linhas.append(linha)
...
with tab_st:
    ...
    if tabela_st.empty:
        ...
        st.dataframe(estilizar_st(tabela_st), use_container_width=True, hide_index=True)
...
with tab_fd:
    ...
    if tabela_fd.empty:
        ...
        st.dataframe(estilizar_fd(tabela_fd), use_container_width=True, hide_index=True)
