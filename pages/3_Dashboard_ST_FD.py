...
# Visual ajustado para ficar fiel ao modelo enviado: Calibri/Arial, valores sem negrito,
# células compactas, grade azul-clara e rolagem horizontal habilitada.
st.markdown(
    """
    <style>
        .block-container {
    ...
    for _, row in tabela.iterrows():
        ...
        for col in tabela.columns:
            if col == "Planta":
                ...
                html += "<td class='valor' style='background:" + bg + ";'>" + fmt_decimal(row[col]) + "</td>"
            else:
                bg = cor_fd_hex(row[col])
                html += "<td class='valor' style='background:" + bg + ";'>" + fmt_percentual(row[col]) + "</td>"
        html += "</tr>"
