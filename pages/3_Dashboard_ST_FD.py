...
def tabela_heatmap_html(tabela, tipo="ST", escala=1.0):
    if tabela.empty:
        return ""
    ...
    css = f"""
    <style>
    .heatmap-wrap {{ overflow-x:auto; width:100%; background:white; border-radius:6px; }}
    table.heatmap-table {{ border-collapse:collapse; font-family:Arial, Helvetica, sans-serif; font-size:{font_size}px; background:white; color:#000000; }}
    .heatmap-table th, .heatmap-table td {{ border:1px solid #D9E2EF; padding:{pad_y}px {pad_x}px; text-align:center; white-space:nowrap; min-width:{min_w}px; height:{int(23*escala)}px; line-height:1.1; font-weight:400; }}
    .heatmap-table th {{ background:#F8FAFC; color:#0F172A; font-weight:600; }}
    .heatmap-table td.planta {{ background:#FFFFFF; color:#000000; font-weight:400; text-align:left; }}
    .heatmap-table td.base {{ background:#FFFFFF; color:#000000; font-weight:400; }}
    </style>
    """
    ...
    for _, row in tabela.iterrows():
        ...
        for col in tabela.columns:
            if col == "Planta":
                ...
                html += "<td style='background:" + bg + ";color:#000000;'>" + fmt_decimal(row[col]) + "</td>"
            else:
                bg = cor_fd_hex(row[col])
                html += "<td style='background:" + bg + ";color:#000000;'>" + fmt_percentual(row[col]) + "</td>"
        html += "</tr>"
...
def render_heatmap_png(tabela, tipo="ST", titulo="Heatmap", escala=1.0):
    if tabela.empty:
        return None
    ...
    cell_w = 1.15 * escala
    cell_h = 0.58 * escala
    fig_w = max(18, n_cols * cell_w)
    fig_h = max(6, (n_rows + 2.2) * cell_h)

    dpi = 260
    fig, ax = plt.subplots(figsize=(fig_w, fig_h), dpi=dpi)
    ax.set_xlim(0, n_cols)
    ax.set_ylim(0, n_rows + 1.55)
    ax.axis("off")
    ...
    ax.text(n_cols / 2, n_rows + 1.18, titulo, ha="center", va="center", fontsize=title_fs, fontweight="bold", color="#0F172A")

    header_y = n_rows
    for c, col in enumerate(tabela.columns):
        ax.add_patch(Rectangle((c, header_y), 1, 0.65, facecolor="#F8FAFC", edgecolor="#D9E2EF", linewidth=0.9))
        ax.text(c + 0.5, header_y + 0.325, str(col), ha="center", va="center", fontsize=header_fs, fontweight="normal", color="#0F172A")
    ...
    for r, (_, row) in enumerate(tabela.iterrows()):
        ...
        for c, col in enumerate(tabela.columns):
            if col == "Planta":
                ...
                bg = "#FFFFFF"
                txt = fmt_decimal(row[col])
            elif tipo == "ST":
                bg = cor_st_hex(row[col], row.get("M300"), row.get("RF"))
                txt = fmt_decimal(row[col])
            else:
                bg = cor_fd_hex(row[col])
                txt = fmt_percentual(row[col])

            ax.add_patch(Rectangle((c, y), 1, 1, facecolor=bg, edgecolor="#D9E2EF", linewidth=0.9))
            ax.text(c + 0.5, y + 0.5, txt, ha="center", va="center", fontsize=cell_fs, fontweight="normal", color="#000000")

    buf = BytesIO()
    fig.savefig(buf, format="png", bbox_inches="tight", pad_inches=0.18, facecolor="white")
    plt.close(fig)
    buf.seek(0)
    return buf.getvalue()
...
with tab_st:
    ...
    if tabela_st.empty:
        ...
        html_st = tabela_heatmap_html(tabela_st, tipo="ST", escala=1.0)
        st.markdown(html_st, unsafe_allow_html=True)
        png_st = render_heatmap_png(tabela_st, tipo="ST", titulo=titulo_st, escala=1.0)
        png_st_grande = render_heatmap_png(tabela_st, tipo="ST", titulo=titulo_st, escala=1.35)
        html_st_print = tabela_heatmap_html(tabela_st, tipo="ST", escala=1.45)
        with st.expander("Visualizacao completa para print", expanded=False):
            st.caption("Esta visualizacao em HTML fica mais nitida para print. Se preferir, baixe o PNG grande abaixo.")
            st.markdown(html_st_print, unsafe_allow_html=True)
        col_download_1, col_download_2, col_download_3 = st.columns(3)
        with col_download_1:
            st.download_button("Baixar ST PNG normal", data=png_st, file_name="st_heatmap.png", mime="image/png")
        with col_download_2:
            st.download_button("Baixar ST PNG grande", data=png_st_grande, file_name="st_heatmap_grande.png", mime="image/png")
        with col_download_3:
            st.download_button("Baixar ST HTML", data=html_document(titulo_st, html_st_print).encode("utf-8"), file_name="st_heatmap.html", mime="text/html")
...
with tab_fd:
    ...
    if tabela_fd.empty:
        ...
        html_fd = tabela_heatmap_html(tabela_fd, tipo="FD", escala=1.0)
        st.markdown(html_fd, unsafe_allow_html=True)
        png_fd = render_heatmap_png(tabela_fd, tipo="FD", titulo=titulo_fd, escala=1.0)
        png_fd_grande = render_heatmap_png(tabela_fd, tipo="FD", titulo=titulo_fd, escala=1.35)
        html_fd_print = tabela_heatmap_html(tabela_fd, tipo="FD", escala=1.45)
        with st.expander("Visualizacao completa para print", expanded=False):
            st.caption("Esta visualizacao em HTML fica mais nitida para print. Se preferir, baixe o PNG grande abaixo.")
            st.markdown(html_fd_print, unsafe_allow_html=True)
        col_download_1, col_download_2, col_download_3 = st.columns(3)
        with col_download_1:
            st.download_button("Baixar FD PNG normal", data=png_fd, file_name="fd_multiflex_heatmap.png", mime="image/png")
        with col_download_2:
            st.download_button("Baixar FD PNG grande", data=png_fd_grande, file_name="fd_multiflex_heatmap_grande.png", mime="image/png")
        with col_download_3:
            st.download_button("Baixar FD HTML", data=html_document(titulo_fd, html_fd_print).encode("utf-8"), file_name="fd_multiflex_heatmap.html", mime="text/html")
