...
def montar_tabela_st(df_mes):
    ...
    for planta in PLANTAS_ORDEM:
        ...
        for _, row in bloco.iterrows():
            linha[row["data_label"]] = row["valor_real"] if pd.notna(row["valor_real"]) else None
...
def tabela_st_html(tabela):
    ...
    table.heatmap-table {border-collapse:collapse; font-family:Arial, sans-serif; font-size:12px; background:white;}
    .heatmap-table th, .heatmap-table td {border:1px solid #CBD5E1; padding:6px 8px; text-align:center; white-space:nowrap; min-width:52px;}
    .heatmap-table th {background:#F8FAFC; color:#0F172A; font-weight:800;}
    .heatmap-table td.planta {background:#EEF2F7; color:#000; font-weight:800; text-align:left;}
    .heatmap-table td.base {background:#EEF2F7; color:#000; font-weight:800;}
    </style>
    """
    ...
    for _, row in tabela.iterrows():
        ...
        for col in tabela.columns:
            if col == "Planta":
                ...
                html += "<td style='" + cor_st(row[col], row.get("M300"), row.get("RF")) + "font-weight:800;'>" + fmt_decimal(row[col]) + "</td>"
            elif col in dias:
                html += "<td style='" + cor_st(row[col], row.get("M300"), row.get("RF")) + "'>" + fmt_decimal(row[col]) + "</td>"
            else:
                html += "<td></td>"
        html += "</tr>"
    ...
    for _, row in tabela.iterrows():
        ...
        for col in tabela.columns:
            if col == "Planta":
                ...
                html += "<td style='" + cor_fd(row[col]) + "'>" + fmt_percentual(row[col]) + "</td>"
        html += "</tr>"
    html += "</tbody></table></div>"
    return html
...
with tab_st:
    ...
    if tabela_st.empty:
        ...
        st.markdown(tabela_st_html(tabela_st), unsafe_allow_html=True)
...
with tab_fd:
    ...
    if tabela_fd.empty:
        ...
        st.markdown(tabela_fd_html(tabela_fd), unsafe_allow_html=True)
