...
try:
    ...
    def aplicar_css_global(max_width="100%", esconder_sidebar=False):
        st.markdown(f"""<style>.block-container{{max-width:{max_width}!important;padding-left:.75rem!important;padding-right:.75rem!important;}}</style>""", unsafe_allow_html=True)
...
def celula_html(texto, classe="", style=""):
    return (
        f"<td class='{classe}' style='font-weight:400 !important; font-family:Calibri, Arial, Helvetica, sans-serif !important; {style}'>"
        f"<span style='font-weight:400 !important; font-family:Calibri, Arial, Helvetica, sans-serif !important;'>{texto}</span>"
        "</td>"
    )
...
def tabela_heatmap_html(tabela, tipo="ST"):
    ...
    for col in tabela.columns:
        html += (
            "<th style='font-weight:400 !important; font-family:Calibri, Arial, Helvetica, sans-serif !important;'>"
            f"<span style='font-weight:400 !important;'>{str(col)}</span></th>"
        )
    html += "</tr></thead><tbody>"
    for _, row in tabela.iterrows():
        ...
        for col in tabela.columns:
            if col == "Planta":
                html += celula_html(str(row[col]), classe="planta")
            elif tipo == "ST" and col in ["M300", "RF"]:
                html += celula_html(fmt_decimal(row[col]), classe="base")
            elif tipo == "ST":
                bg = cor_st_hex(row[col], row.get("M300"), row.get("RF"))
                html += celula_html(fmt_decimal(row[col]), classe="valor", style=f"background:{bg};")
            else:
                bg = cor_fd_hex(row[col])
                html += celula_html(fmt_percentual(row[col]), classe="valor", style=f"background:{bg};")
        html += "</tr>"
