...
from openpyxl import load_work(3)), int(m.group(2)), int(m.group(1))).date()from openpyxl import load_workbook, Workbook
        except ValueError:
            pass
        ...
        try:
            ...
            if detectar_consolidado_existente(item):
                ...
                if data_arquivo:
                    ...
                    datas_encontradas.setdefault(
                        data_arquivo,
                        {
                            "data": datetime.combine(data_arquivo, datetime.min.time()),
                            "arquivos": [],
                            "abas": []
                        }
                    )
                    datas_encontradas[data_arquivo]["arquivos"].append(item["nome"])
                    datas_encontradas[data_arquivo]["abas"].extend(
                        [a for a in wb.sheetnames if identificar_aba_padrao(a)]
                    )
            ...
            for reg in listar_datas_do_arquivo(wb, ano_ref, mes_ref):
                ...
                datas_encontradas.setdefault(
                    data_real,
                    {
                        "data": datetime.combine(data_real, datetime.min.time()),
                        "arquivos": [],
                        "abas": []
                    }
                )
            ...
            if not encontrou:
                sem_data.append({
                    "arquivo": item["nome"],
                    "motivo": "Nenhuma data identificável por nome da aba ou J3/K3"
                })
...
def processar_data(data_processar, candidatos, copiar_estilos=True, callback_progresso=None):
    ...
    logs.append(
        "Se houver Gerencial para a data em um arquivo, usa somente Gerencial desse arquivo; "
        "para datas históricas, usa a aba histórica correspondente."
    )
    ...
    for cand in candidatos:
        ...
        try:
            ...
            logs.append(
                f"SUCESSO: {cand['arquivo']} | "
                f"'{cand['aba_origem']}' -> '{cand['aba_final']}' ({cand['tipo']})."
            )

        except Exception as erro:
            arquivos_com_erro.append({
                "arquivo": cand.get("arquivo", "arquivo"),
                "erro": str(erro)
            })
...
def processar_multiplas_datas(datas_selecionadas, arquivos_salvos, data_referencia, copiar_estilos=True):
    ...
    candidatos_por_data = {
        data: candidatos_para_data(data, arquivos_salvos, data_referencia)
    ...
    def progresso(nome_arquivo, data_atual):
        ...
        tempo_box.info(
            ...
            f"Progresso: {concluidos}/{total_etapas} aba(s)"
        )
    ...
    for idx, data in enumerate(datas_ordenadas, start=1):
        ...
        status.info(
            f"Gerando consolidado {idx}/{len(datas_ordenadas)}: "
            f"{data.strftime('%d/%m/%Y')} com {len(candidatos)} aba(s)."
        )

        resultado = processar_data(
            data,
            candidatos,
            copiar_estilos=copiar_estilos,
...
usar_data_ontem = st.sidebar.checkbox(
    "Usar ontem como referência de mês/ano",
    value=True
)
...
if arquivos_salvos:
    ...
    if len(arquivos_salvos) != qtde_esperada:
        st.warning(
            f"Foram carregados {len(arquivos_salvos)} arquivo(s), "
            f"mas a quantidade esperada é {qtde_esperada}."
        )
    ...
    with st.expander("Ver arquivos carregados"):
        for i, item in enumerate(arquivos_salvos, start=1):
            ...
            st.write(
                f"{i}. {item['nome']} — "
                f"tipo: {tipo} — "
                f"aba final prevista: {nome_padronizado}"
            )
else:
...
if arquivos_salvos:
    datas_para_filtro, sem_data = construir_indice_datas(
        arquivos_salvos,
...
if botao_processar:
    try:
        ...
        # =====================================================
        # NOVO: SALVA OS CONSOLIDADOS NO HISTÓRICO DO DASHBOARD
        # =====================================================
        ...
        if len(resultados) > 1:
            ...
            st.download_button(
                "⬇️ Baixar todos os consolidados em ZIP",
                zip_resultados,
                "Consolidados_Gerenciais_CN.zip",
        ...
        for resultado in resultados:
            ...
            st.download_button(
                f"⬇️ Baixar {resultado['nome_arquivo_final']}",
...
st.caption(
    "Cada consolidado contém apenas abas da data selecionada. "
    "Para a mesma data dentro de um arquivo: Gerencial tem prioridade e é exclusiva. "
    "Para datas históricas diferentes: usa a aba histórica correspondente. "
