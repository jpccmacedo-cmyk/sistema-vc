...
with tab_map["Análise Executiva"]:
    ...
    if df_status_exec.empty:
        ...
        ranking_plantas = df_status_exec.sort_values("Score", ascending=False).copy()
        ranking_plantas["Posição"] = range(1, len(ranking_plantas) + 1)
        ranking_plantas["Score"] = ranking_plantas["Score"].map(lambda x: f"{x:.1f}%".replace(".", ","))
        colunas_ranking = [
            "Posição",
            "Codigo",
            "Nome",
        ...
        colunas_ranking = [col for col in colunas_ranking if col in ranking_plantas.columns]
        st.dataframe(ranking_plantas[colunas_ranking], use_container_width=True, hide_index=True)
