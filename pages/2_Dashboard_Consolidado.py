...
def ajustar_resultado(indicador, valor):
    ...
    # KKC deve ficar na escala 0-100.
    # Exemplo: 0,55 vira 55.
...
def cor_status(status):
    if status == "verde":
        return "background-color: #00B050; color: white; font-weight: 800"

    if status == "vermelho":
        return "background-color: #FF0000; color: white; font-weight: 800"

    if status == "informativo":
        return "background-color: #D9D9D9; color: black; font-weight: 700"

    if status == "sem_meta":
        return "background-color: #F3F4F6; color: #6B7280"

    return "background-color: #CFCFCF; color: black; font-weight: 700"
...
with tab_farol:
    st.markdown(
        "<h2 style='color:#111827; font-weight:900; margin-bottom:4px;'>"
        "Farol de Indicadores Mensais CN - "
        + data_farol
        + "</h2>",
    ...
    st.caption(
        ...
        "mas MTD CN permanece NA e nao entra no calculo de status."
    )

    html_farol = estilizar_tabela_farol(df_tabela_farol, status_celulas).to_html()

    st.markdown(
        "<div style='overflow-x:auto; padding-bottom:12px;'>" + html_farol + "</div>",
        unsafe_allow_html=True
    )

with tab_status:
    st.markdown(
        "<h2 style='text-align:center; color:#1A2A8F; "
    ...
    st.caption(
        "CN nao e calculado neste grafico. "
    ...
    if df_status.empty:
        ...
        df_status_exibir["Acima Meta MTD"] = df_status_exibir["Acima Meta MTD"].map(
            lambda x: f"{x:.2f}%".replace(".", ",")
        )
        df_status_exibir["Abaixo Meta MTD"] = df_status_exibir["Abaixo Meta MTD"].map(
            lambda x: f"{x:.2f}%".replace(".", ",")
        )
