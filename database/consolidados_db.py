...
def _connect_from_url(url):
    """
    Conecta a partir de URL. Se psycopg2 não aceitar diretamente,
    tenta quebrar a URL em parâmetros.
    """
    url = _normalizar_url(url)
    if not url:
        return None

    # Primeira tentativa: DSN/URL direto.
    try:
        if "sslmode=" in url:
            return psycopg2.connect(url)
        return psycopg2.connect(url, sslmode="require")
    except Exception:
        # Segunda tentativa: urlparse. Ajuda quando a URL precisa de ajustes simples.
        parsed = urlparse(url)
...
def get_connection():
    """
    Aceita estes formatos de secrets:

    1) DATABASE_URL = "postgresql://user:password@host/db?sslmode=require"
    ...
    # Streamlit secrets: blocos comuns.
    ...
    try:
        with conn.cursor() as cur:
            for resultado in resultados:
                ...
                # Correção: remove o antigo da mesma data antes de gravar o novo.
