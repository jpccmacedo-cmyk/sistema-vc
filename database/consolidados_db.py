...
def _get_column_types(conn, schema_name, table_name):
    """Retorna tipos das colunas no formato {nome_coluna: data_type}."""
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute(
            """
...
def _tipo_coluna(cfg, logical_name):
    """Obtém o tipo físico da coluna mapeada, se disponível."""
    try:
        coluna = cfg["mapping"].get(logical_name)
        if not coluna:
            return None
        return cfg.get("types", {}).get(coluna)
    except Exception:
        return None


def _data_para_banco(data_processada, cfg=None):
    """
    Define como gravar a data conforme o tipo da coluna.
    - Se a coluna for date/timestamp: grava date.
    - Se a coluna for text/varchar: grava 'YYYY-MM-DD'.
    """
    data_proc = _normalizar_data_processada(data_processada)
    tipo = _tipo_coluna(cfg, "data") if cfg else None
    if tipo in ["date", "timestamp without time zone", "timestamp with time zone"]:
        return data_proc
    return data_proc.strftime("%Y-%m-%d")


def _data_variantes_texto(data_processada):
    """Variações textuais usadas para localizar datas salvas como texto."""
    data_proc = _normalizar_data_processada(data_processada)
    return [
        data_proc.strftime("%Y-%m-%d"),
        data_proc.strftime("%d/%m/%Y"),
        data_proc.strftime("%d.%m.%Y"),
    ]
...
def _descobrir_tabela_consolidados(conn):
    ...
    for row in rows:
        ...
        try:
            ...
            types = _get_column_types(conn, schema_name, table_name)
    ...
    types = _get_column_types(conn, DEFAULT_SCHEMA, DEFAULT_TABLE)
    return {
...
def _excluir_mesma_data(cur, cfg, data_processada, ano, mes):
    ...
    Correção importante:
    algumas bases antigas têm data_processada como TEXT, enquanto outras têm DATE.
    Para evitar erro "operator does not exist: text = date", a comparação é feita
    com a coluna convertida para texto, aceitando formatos comuns de data.
    """
    m = cfg["mapping"]
    datas_texto = _data_variantes_texto(data_processada)
...
def _insert_consolidado(cur, cfg, nome_arquivo, dados, data_proc, ano, mes):
    ...
    data_valor = _data_para_banco(data_proc, cfg)
    vals = [nome_arquivo, psycopg2.Binary(dados), data_valor, int(ano), int(mes)]
...
def carregar_arquivo_consolidado(consolidado_id):
    """
    Carrega o arquivo consolidado pelo ID.

    Importante:
    - Não converte consolidado_id para int.
    - Bases antigas podem usar UUID, TEXT ou outro tipo de chave.
    - O psycopg2 faz o bind do tipo correto no parâmetro %s.
    """
    conn = get_connection()
    try:
        cfg = _descobrir_tabela_consolidados(conn)
        ...
        with conn.cursor() as cur:
            cur.execute(query, (consolidado_id,))
            row = cur.fetchone()
        if row is None:
            return None, None
...
def excluir_consolidado(consolidado_id):
    """
    Exclui consolidado pelo ID, sem converter para int.
    Compatível com ID numérico, UUID ou texto.
    """
    conn = get_connection()
    try:
        cfg = _descobrir_tabela_consolidados(conn)
